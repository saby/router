...
export default class History implements IHistory {
    ...
    constructor(
        ...
        this._setSpaHistory = typeof setSpaHistory === 'function' ? setSpaHistory : () => {};
        this._initHistory();
    }
    ...
    forward(newState?: IHistoryState): void {
        ...
        if (newHistoryPosition === history.length) {
            const newHistoryState: IHistoryState = this._calculateNewHistoryState(
                newState as IHistoryState
            );
            history.push({
...
if (typeof window !== 'undefined') {
    ...
    window.history.replaceState = function () {
        ...
        origReplaceState.apply(window.history, arguments);
    };
}
