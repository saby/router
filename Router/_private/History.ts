...
export default class History implements IHistory {
    ...
    private _initHistory(): void {
        let currentUrl: string = this._routerUrl.getUrl();
        // если открывать url с двойным слешем в начале (напр. "//page/some"), то браузер всё разрулит корректно.
        // т.к. в при инициализации начального состояния есть запись в window.history,
        // то падает window.history.replaceState({}, '//page/some') с SecurityError
        // поэтому двойной слеш превратим в одинарный
        currentUrl = currentUrl.replace(/^[\/]{2,}/, '/');

        const initialState: IHistoryState = {
            id: 0,
            state: this._urlRewriter.get(this._routerUrl.getLogicUrl()),
            href: currentUrl,
        };
    ...
    forward(newState?: IHistoryState): void {
        ...
        // если переход вперед после самого последнего (правого) перехода, то новый переход добавим в историю
        ...
        // при переходе вперед актуальный объект истории берется из браузера
        // - он может отличаться от того, что сохранено в истории роутера, т.к. где-то могли позвать history.replaceState в процессе перехода.
        // поэтому обновим текущее состояние в истории роутера значением, которое пришло из браузера
        else if (newState && newState.state && newState.href && newState.id) {
            history[newHistoryPosition] = newState;
        }
    ...
    replaceState(newState: IHistoryState): void {
        ...
        newState.href =
            newState.href ||
            this._routerUrl.getRealUrl(this._urlRewriter.getReverse(newState.state));
