...
import {IParam, PathParams, QueryParams} from './UrlParamsManager';
import {IUrlParts, UrlPartsManager} from './UrlPartsManager';

/**
 *
 */
export class UrlModifyManager {
    private mask: string;
    private url: string;
    ...
    rewrite(): string {
        switch (this.maskType) {
            case MaskType.Path:
                this.urlParts.path = PathModifier.rewrite(this.mask, this.cfg, this.urlParts.path);
                break;
            case MaskType.Query:
                this.urlParts.query =  QueryModifier.rewrite(this.mask, this.cfg, this.urlParts.query);
                break;
            case MaskType.PathFragment:
                this.urlParts.fragment = PathModifier.rewrite(this.mask, this.cfg, this.url);
                break;
            case MaskType.QueryFragment:
                this.urlParts.fragment = QueryModifier.rewrite(this.mask, this.cfg, this.url);
                break;
...
class PathModifier {
    static rewrite(mask: string, cfg: Record<string, unknown>, urlPart: string): string {
