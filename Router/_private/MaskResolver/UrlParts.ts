...
export class UrlParts {
    ...
    private _parseUrl(url: string): IUrlParts {
        ...
        let extQuery = '';
        if (query) {
            extQuery = extractExtQuery(query);
        }
    ...
    join(newUrlParts: IUrlParts, maskHasTrailingSlash: boolean = false): string {
        ...
        if (extQuery && !extractExtQuery(query)) {
            if (query.length > 0) {
 ...
 * @returns string
 */
function extractExtQuery(url: string): string {
    const regex = /[?&](ext[^=]+)=([^&]+)/g;
    let matches = [];
...
// на клиенте подменяем window.open для того, чтобы в url в параметре ret добавить ext-параметры
if (typeof window !== 'undefined') {
    const origOpen = window.open;
    const retRegex = /ret=([^&]+)/;
    // @ts-ignore
    window.open = function (url: string, target?: string, windowFeatures?: string) {
        try {
            if (!url || target !== '_blank') {
                return origOpen(url, target, windowFeatures);
            }

            const extQuery = extractExtQuery(window.location.href);
            if (!extQuery) {
                return origOpen(url, target, windowFeatures);
            }

            const match = url.match(retRegex);
            if (!match || !match[1]) {
                return origOpen(url, target, windowFeatures);
            }

            const retParam = decodeURIComponent(match[1]);
            const join = retParam.indexOf('?') >= 0 ? '&' : '?';
            const replacePart = encodeURIComponent(retParam + join + extQuery);
            const newUrl = url.replace(retRegex, `ret=${replacePart}`);
            return origOpen(newUrl, target, windowFeatures);
        } catch (err) {
            /* eslint-disable-next-line no-console */
            console.error('Произошла ошибка при попытке вставить в новый url ext-параметры ', err);
        }
        return origOpen(url, target, windowFeatures);
    };
}
