...
export class PathParams {
    ...
    static calculateParams(mask: string, urlPart: string): IParam[] {
        ...
        const maskEntries: [string, string[]][] = Array.from(PathParams._parseMask(mask).entries());
        maskEntries.forEach(([urlId, maskIds]: [string, string[]], entryIndex: number) => {
            // следующий литерал маски за текущей группой параметров, напр. 'uc' в маске /:city/uc.
            // нужен для выравнивания параметров без urlId по литералу в url
            let nextLiteral: string | undefined;
            for (let i: number = entryIndex + 1; i < maskEntries.length; i++) {
                if (typeof maskEntries[i][0] !== 'undefined') {
                    nextLiteral = maskEntries[i][0];
                    break;
                }
            }
     ...
     * @param nextLiteral следующий за группой параметров литерал маски, напр. 'uc' в маске /:city/uc
     * @private
     */
    protected static _getUrlValue(
        ...
        let urlIdIndex: number | undefined;
        if (typeof urlId === 'undefined') {
            ...
            if (typeof nextLiteral !== 'undefined') {
                // выравниваем параметры без urlId по литералу маски в url:
                // берем valuesCount сегментов непосредственно перед литералом, префикс url сохраняем
                // напр. mask=/:city/uc и url=/path/uchaly/uc -> city=uchaly, а /path останется
                const literalIndex: number = urlPartsArray.indexOf(nextLiteral);
                if (literalIndex > 0) {
                    startIndex = Math.max(0, literalIndex - valuesCount);
                }
            }
            urlValues = urlPartsArray.splice(startIndex, valuesCount);
        } else {
