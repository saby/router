 ...
 * @interface Router/_private/MaskResolver/IUrlParams
 * @public
 */
 ...
 * @interface Router/_private/MaskResolver/IUrlQueryParams
 * @public
 */
...
export class UrlModifier {
    ...
    constructor(mask: string, cfg: Partial<IUrlParams>, url: string) {
        ...
        // определим тип маски
        this.maskTypes = calculateMaskType(
            mask,
            this.urlParts,
            cfg.trailingSlash
        );
    ...
    modify(): string {
        ...
        this.maskTypes.forEach((maskType) => {
            switch (maskType.maskType) {
                ...
                case MaskType.Query:
                    newUrlParts.query =
                        QueryModifier.createQueryObject().modify(
                            maskType.mask,
                            this.cfg,
                ...
                case MaskType.QueryFragment:
                    newUrlParts.fragment =
                        QueryModifier.createFragmentObject().modify(
                            maskType.mask,
                            this.cfg,
                            isReplaceMask ? '' : this.urlParts.getFragment()
                        );
...
class PathModifier {
    static modify(
        ...
        const newPath: string = PathModifier._calculateNewUrlPart(
            urlPart,
            mask,
            params
        );
     ...
     * @private
     */
    private static _calculateNewUrlPart(
        urlPart: string,
        mask: string,
        params: IParam[]
    ): string {
        ...
        let urlPartFromMask: string = mask.replace(
            new RegExp(reMaskReplace),
            ''
        );
        ...
        let newUrlPart: string = urlPart.slice(
            0,
            maskIndexInUrl + urlPartFromMask.length
        );
        const urlPartSliced: string = urlPart.slice(
            maskIndexInUrl + urlPartFromMask.length
        );
        reUrlReplaceItems.forEach((reUrlReplace) => {
            const _match: RegExpMatchArray = urlPartSliced.match(
                new RegExp(reUrlReplace)
            );
            ...
            if (_match) {
                ...
                const matchRightChar =
                    urlPartSliced[matchIndex + _match[0].length];
                const matchLastChar = _match[0][_match[0].length - 1];
                if (
                    (matchIndex === 0 ||
                        urlPartSliced[matchIndex - 1] === '/') &&
                    (typeof matchRightChar === 'undefined' ||
                        matchLastChar === '/')
                ) {
                    newUrlPart += _match[0];
                }
    ...
    private static _getUrlIdIndex(
        ...
        while (true) {
            ...
            if (
                urlPartsArray[index + 1] &&
                urlPartsArray[index + 1] === urlValue
            ) {
                return index;
            }
            prevIndex = index;
...
class QueryModifier {
    ...
    modify(
        ...
        const params: IParam[] = this.queryParams.calculateParams(
            mask,
            urlPart
        );
