...
export class UrlModifier {
    ...
    constructor(mask: string, cfg: Partial<IUrlParams>, url: string) {
        ...
        if (this.cfg.replace) {
            const extQuery = this.urlParts.getExtQuery();
            this.urlParts = new UrlParts('');
            this.urlParts.setExtQuery(extQuery);
...
class PathModifier {
    static modify(mask: string, cfg: Record<string, unknown>, urlPart: string): string {
        ...
        // индексы в массиве частей url, которые были ранее заменены.
        // нужно чтобы не заменить уже замененную часть, т.к. значения могут совпасть
        ...
        for (let i = 0; i < params.length; i++) {
            ...
            if (param.urlId) {
                ...
                replaceIndex = PathModifier._getReplaceIndex(
                    newPathArray,
                    param.urlValue,
                    replacedIndexes
                );
            }
            replacedIndexes.push(replaceIndex);
    ...
    private static _calculateNewUrlPart(urlPart: string, mask: string, params: IParam[]): string {
        ...
        params.forEach((param) => {
            if (param.urlId) {
                ...
                // После `param.urlId` добавлена ​​опережающая проверка `(?=\/|$)`, чтобы предотвратить совпадение
                // литерала `urlId` (например, `menu`) как подстроки внутри более длинного сегмента пути (например, `menushop`):
        ...
        let maskIndexInUrl: number = urlPart.indexOf(urlPartFromMask);
        if (!urlPartFromMask.endsWith('/')) {
            while (maskIndexInUrl !== -1) {
                const charAfter: string = urlPart[maskIndexInUrl + urlPartFromMask.length];
                if (
                    charAfter === undefined ||
                    charAfter === '/' ||
                    charAfter === '?' ||
                ...
                maskIndexInUrl = urlPart.indexOf(urlPartFromMask, maskIndexInUrl + 1);
            }
        }
        if (maskIndexInUrl === -1) {
     ...
     * @param replacedIndexes   индексы в массиве частей url, которые были ранее заменены, нужно чтобы не заменить уже замененную часть,
     *                          т.к. значения могут совпасть
     * @private
     */
    ...
    private static _getReplaceIndex(
        newPathArray: string[],
        urlValue: string,
        replacedIndexes: number[]
    ): number {
        ...
        let prevIndex = -1;
        while (true) {
            replaceIndex = newPathArray.indexOf(urlValue, prevIndex + 1);
            if (replaceIndex === -1) {
                return -1;
            }
            if (replacedIndexes.includes(replaceIndex)) {
