 ...
 * @interface Router/_private/IUrlRewriter
 * @public
 * @author Мустафин Л.И.
 */
...
export default class UrlRewriter implements IUrlRewriter {
    ...
    static getInstance(): UrlRewriter {
        if (!UrlRewriter.instance) {
            const routes = _prepareRoutes(
                (routerJson as unknown as Record<string, string>) || {}
            );
...
function _getBestMatchFromRouteTree(url: string, rootNode: IRouteTree): string {
    ...
    const setLastFoundResult = (state, index, regexValues = []) => {
    ...
    for (let i = 0; i < urlParts.length; i++) {
        ...
        const { regexValues, state, treeNode } = _getRegExpUrlPart(
            urlPart,
            curTreeNode
        );
        if (!treeNode) {
        ...
        routeNameParts.forEach((part, i) => {
            ...
            if (i === routeNameParts.length - 1) {
                if (
                    ...
                    curTreeNode[part].value.length > routeDest.length
                ) {
