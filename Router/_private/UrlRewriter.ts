...
export default class UrlRewriterAPI implements IUrlRewriter {
    ...
    static getInstance(): UrlRewriterAPI {
        ...
        if (!instance) {
            ...
            // базовый router.json текущей сборки - его добавляем первым, чтобы он всегда обрабатывался первым
...
function _buildRouteTree(entries: RouteEntriesArray): IRouteTree {
    ...
    entries.forEach((entry) => {
        ...
        // маршрут вида /path/foo или /path/regex:^abc$ превращает
        // в массив вида ['path', 'foo'] или ['path', 'regex:^abc$']
        const routeNameParts: string[] = routeName.split('/').filter((part) => part);
