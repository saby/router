    ...
    private _beforeApplyNewUrl(
        ...
        const applyNewUrlOptions = (
            ...
            if (hasResolvedParam && !this._isResolved) {
                // @ts-ignore
                const eventObject = new SyntheticEvent({});
                // @ts-ignore
                result = this.props.onEnter?.(eventObject, newLoc, oldLoc);
                result = result ?? (eventObject.result as boolean);
                this._isResolved = true;
            } else if (!hasResolvedParam && this._isResolved) {
                // @ts-ignore
                const eventObject = new SyntheticEvent({});
                // @ts-ignore
                result = this.props.onLeave?.(eventObject, newLoc, oldLoc);
                result = result ?? (eventObject.result as boolean);
        ...
        const eventObject = new SyntheticEvent({});
        // @ts-ignore
        let result = this.props.onBeforeChange?.(eventObject, newLoc, oldLoc);
        result = result ?? eventObject.result;
