...
import { IProgressBar, IRouteProps } from './shared/IRouteProps';
import { getUrlOptionsFromProps } from './shared/getUrlOptionsFromProps';
 ...
 * <a href="https://link.sbis.ru/article/9e0b0406-295b-4bc9-a53e-693244dc7bad#toc_9ddbd76a-774b-4161-b538-532f9810fd05">Статья о компоненте</a>
 *
 * @example
 * <pre>
 ...
 *     return (
 *         <Route mask="destination/:country">
 *             <Inner />
 *         </Route>
 *     );
 * }
 *
 * function Inner(props: { country?: string }): JSX.Element {
 *     return <p>Selected destination: {props.country}</p>;
 * }
 * </pre>
 *
 * @author Мустафин Л.И.
...
class Route extends Component<IRouteProps> implements IRegisterableComponent {
    ...
    componentWillUnmount() {
        ...
        if (this._isResolved && this.props.onLeave) {
            this.props.onLeave(this._newLoc, this._oldLoc);
        }
    }
    ...
    private _register(): void {
        this.Router._manager.addRoute(
            ...
            (newLoc: IHistoryState, oldLoc: IHistoryState, notRunCallbacks?: boolean) => {
                return this._beforeApplyNewUrl(newLoc, oldLoc, notRunCallbacks);
            },
            () => {
                this._afterApplyNewUrl();
                return true;
    ...
    private _afterApplyNewUrl(): void {
        ...
        this.props
            .getDataToRender(this._urlOptions)
            .then((pageConfig: unknown) => {
                // пока вызываем getDataToRender пользователь мог уже кликнуть по другой ссылке
            ...
            .catch((err) => {
                logger.warn('Ошибка при загрузке данных', err);
                return err;
            });
    }
    ...
    private _fillUrlOptionsFromOptions(options: IRouteProps & Record<string, unknown>): void {
        this._urlOptions = {
            ...this._urlOptions,
            ...getUrlOptionsFromProps(options, this._urlOptions),
        };
    }
    ...
    private _checkUrlResolved(): void {
        ...
        if (hasResolvedParam) {
            ...
            if (!prevState) {
                prevState = {
                    state: _router.maskResolver.calculateState(this.props.mask, { clear: true }),
                };
            }
            // @ts-ignore
        ...
        for (i in newOptions) {
            if (newOptions.hasOwnProperty(i)) {
                if (!oldOptions.hasOwnProperty(i) || newOptions[i] !== oldOptions[i]) {
                    if (
                        i === 'pageUrl' &&
                        ((typeof newOptions[i] === 'undefined' && oldOptions[i] === 'main') ||
                            (newOptions[i] === 'main' && typeof oldOptions[i] === 'undefined'))
                    ) {
                        // TODO https://online.sbis.ru/opendoc.html?guid=0a8f08be-c368-4dc3-8f50-f88f7856f09a&client=3
                        continue;
                    }
                    return true;
                }
