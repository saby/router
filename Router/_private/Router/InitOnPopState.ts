...
export default function initOnPopState(routerGetter: () => IRouter): void {
    ...
    window.onpopstate = (event: PopStateEvent) => {
        ...
        // в ситуациях, когда:
        // 1) в url руками добавляют фрагмент/якорь и жмут Enter
        // 2) клик по <a/>, в котором в href просто выставляется фрагмент/якорь
        // - тогда срабатывает это событие с пустым state
        // такое обрабатывать нельзя
        if (!event.state) {
            return;
        }
        ...
        if (
            ...
            (event.state && event.state.id < (currentState.id as number))
        ) {
            const navigateToState: IHistoryState = _getNavigationState(
                prevState,
                event.state,
                event.state || prevState ? router.url.getStateUrl() : router.url.getUrl()
            );
