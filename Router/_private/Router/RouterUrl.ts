...
export default class RouterUrl implements IRouterUrl {
    ...
    getServiceUrl(url?: string): string {
        ...
        const result = '/' + hrefParts.join('/');
        logRouter('getServiceUrl.result', result);
        return result;
    }
     ...
     */
    getRealUrl(href?: string): string {
        logRouter('getRealUrl.href', href);
        let _href = href;
        ...
        if (_href) {
            ...
            logRouter('getRealUrl.alias', _href);
        } else {
            // если href НЕ указан, тогда просто возьмём текущий url
            _href = this.getUrl();
            logRouter('getRealUrl.getUrl', _href);
        }

        // нужно добавить название сервиса в начало
        const result = this.getServiceUrl(_href);
        logRouter('getRealUrl.result', result);
        return result;
    }
}
