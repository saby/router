...
import { IHttpResponse, logger, setConfig } from 'Application/Env';
...
export class PageSourceData {
    ...
    async getResult(options: IRenderOptions, response: IHttpResponse): Promise<TPageSourceData> {
        ...
        try {
            const pageUrlInfo =
                typeof this.appAliasGetter === 'function'
                    ...
                    : undefined;
            if (pageUrlInfo) {
                appAlias = pageUrlInfo.alias;
                pageId = pageUrlInfo.pageId;
        ...
        initResponseWrapper(response, { ...options, pageId });
        ...
        const moduleName = this.moduleNameGetter(logicUrl);
