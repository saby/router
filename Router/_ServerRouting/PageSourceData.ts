...
export class PageSourceData {
    ...
    constructor(
        ...
        private appAliasGetter?: (url: string, router: IRouter) => Promise<string>,
     ...
     */
    async getResult(options: IRenderOptions): Promise<TPageSourceData> {
        try {
            const appAlias =
                typeof this.appAliasGetter === 'function'
                    ? await this.appAliasGetter(this.request.path, options.Router as IRouter)
                    : undefined;
