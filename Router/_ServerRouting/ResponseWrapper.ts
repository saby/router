...
import {
    ...
    initPageRenderer,
    logPartialSend,
    isLoggingPartialSend,
} from 'Router/Builder';
import type { IModuleFound } from './Interfaces/IModuleLoader';
 ...
 * @hidden
 */
export function initResponseWrapper(
    res: IHttpResponse,
    ...
    initPageRenderer(streamResponse, options, loadedModule, pageId);
}
...
function getResponseWrapper(): IRouterResponseWrapper | null {
    ...
    if (!instance) {
        logger.warn(
            'Попытка использовать класс потокового построения без инициализации.',
            new Error().stack
        );
    }
    return instance;
