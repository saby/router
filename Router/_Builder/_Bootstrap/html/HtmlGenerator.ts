...
document.addEventListener('DOMContentLoaded', function () {
    ${getBaseStartScript(requiredModules, mainStart, constants.isProduction ? consoleMessage : '')}
});
         </script>`,
    ].join('');
}

function getBaseStartScript(
    dependencies: string,
...
function getRenderTimeScript(values: IRenderFullData): string {
    ...
    return `<script type="text/javascript">window['ssrTime'] = ${ssrTime};</script>`;
}
