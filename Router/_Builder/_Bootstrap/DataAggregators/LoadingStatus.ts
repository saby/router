...
const onErrorHandler = prepareScript(`
    function onErrorHandler(e) {
        ...
        if (!rid) {
            if (name.indexOf(location.hostname) < 0 &&
                (!wsConfig.staticDomains ||
                    !wsConfig.staticDomains.domains ||
                    !wsConfig.staticDomains.domains.length ||
                    name.indexOf(wsConfig.staticDomains.domains[0]) < 0)
            ) {
                return;
            }
        }
            ...
            if (window.sessionStorage.getItem('${storageKey}')) {
                ...
                if (!window.wsConfig.loadingStatus.dialogShown) {
                    setTimeout(function () {
                        ...
                        if (res) {
                            if (window.onLoadResourceError) {
                                try  {
                                    window.onLoadResourceError(name).then(function() {
 ...
 * *
 * Была зарегистрирована ошибка в логах, в которой у нас НЕ ЗАГРУЗИЛСЯ супурбандл(ы).
