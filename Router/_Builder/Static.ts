...
interface IRenderBuilderOptions extends IRenderOptions, IBuilderOptions {}

/**
 * Рендер html для статичных страниц, которые генерятся из файлов вида name.html.tmpl в builder
 */
export function renderStatic(
    options: IRenderBuilderOptions
...
function renderFullStatic(options: IRenderBuilderOptions): Promise<string> {
    return new Promise((resolve, reject) => {
        require(options.dependencies, () => {
            const moduleName =
                options.extraInfo?.topLevelComponentName ||
                options.dependencies[0];

            renderControls(moduleName, options).then((controlsHTML: string) => {
                ...
                resolve(render({ ...fullData, controlsHTML, moduleName }));
            });
        }, (err) => {
            logger.error(err);
            reject(err);
