...
import { EmptyDemoWsConfig, WsConfig } from './_Bootstrap/DataAggregators/WsConfig';
...
function renderStatelessStatic(options: IRenderBuilderOptions): string {
    ...
    const fullData = new DataAggregator()
        ...
        .add(new WsConfig(_options, true))
        .add(new LoadingStatus())
        .add(new DefaultTags())
        .add(new Head(_options.theme))
...
function renderEmptyDemoStatic(options: IRenderBuilderOptions): string {
    ...
    const fullData = new DataAggregator()
        .addPre(new PageModuleName(moduleName))
        .add(new EmptyDemoWsConfig(options))
        .add(new EmptyDemoBaseScripts())
        .add(new Head(options.theme))
        .getData() as IFullData;
...
function aggregateFullData(
    ...
    let staticPageBundles: IBuilderModuleDeps | undefined;
    ...
    const aggregatedData = new DataAggregator()
        ...
        .add(new JS())
        .add(new Other())
        .getData(true, staticPageBundles);
