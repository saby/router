...
import { IFullData, IRenderOptions, IBuilderOptions } from './_Bootstrap/Interface';
import { renderControls } from './_Bootstrap/renderControls';
import { renderEmptyDemoHTML } from './_Bootstrap/html/renderHTML';
import { renderHTML } from './_Bootstrap/html/HtmlGenerator';
import { DataAggregator } from './_Bootstrap/DataAggregator';
import { BaseScripts, EmptyDemoBaseScripts } from './_Bootstrap/DataAggregators/BaseScripts';
...
import {
    EmptyDemoHeadMainScripts,
    HeadMainScripts,
} from './_Bootstrap/DataAggregators/HeadMainScripts';
import { PageModuleName } from './_Bootstrap/DataAggregators/PageModuleName';
...
export function renderStatic(options: IRenderBuilderOptions): Promise<string> | string {
    ...
    if (options.buildEmptyDemo) {
        return renderEmptyDemoStatic(options);
    }
...
function renderStatelessStatic(options: IRenderBuilderOptions): string {
    ...
    const withPlaceholders = !!options.buildStateless?.withPlaceholders;
    let _options = {
        ...options,
        // эти поля записываем в виде плейсхолдеров. при отдаче от СП плейсхолдеры будут заменены на реальные значения
        wsRoot: '{%WS_ROOT%}',
        resourceRoot: '{%RESOURCE_ROOT%}',
        metaRoot: '{%META_ROOT%}',
        servicesPath: '{%DEFAULT_SERVICE_URL%}',
        appRoot: '{%APP_ROOT%}',
        staticDomains: '{%STATIC_DOMAINS%}',
        shardDomain: '{%SHARD_DOMAIN%}',
        product: '{%PRODUCT%}',
        cdnRoot: '{%CDN_ROOT%}',
        // от билдера приходит %{RUM_ENABLED} - просто отключаем RUM
        RUMEnabled: false,
        // от билдера приходит %{PAGE_NAME} - пишем просто пустоту
        pageName: '',
    };
    if (!withPlaceholders) {
        // это старый способ генерации подобных страниц, будет удалено по задаче
        // @todo https://online.sbis.ru/opendoc.html?guid=01a0d8eb-fdaa-73c4-a5d4-9fce9ae6c966&client=3
    ...
    const fullData = new DataAggregator()
        .addPre(new PageMainDeps())
        .addPre(new PageModuleName(moduleName))
        .add(new HeadMainScripts(_options, !withPlaceholders))
        .add(new DefaultTags())
        .add(new Head())
...
/**
 * Рендер "пустой" статичной страницы для демки require.
 */
function renderEmptyDemoStatic(options: IRenderBuilderOptions): string {
    // moduleName в этом случае не интересен
    const moduleName = '';

    const fullData = new DataAggregator()
        .addPre(new PageModuleName(moduleName))
        .add(new EmptyDemoHeadMainScripts(options))
        .add(new EmptyDemoBaseScripts())
        .add(new Head())
        .getData() as IFullData;

    return renderEmptyDemoHTML(fullData);
}
 ...
 */
function aggregateFullData(options: IRenderOptions, moduleName: string): IFullData {
    const aggregatedData = new DataAggregator()
        .addPre(new PageModuleName(moduleName))
        .add(new HeadMainScripts(options))
        .add(new DefaultTags())
        .add(new Head())
        .add(new Body())
        .add(new BaseScripts())
