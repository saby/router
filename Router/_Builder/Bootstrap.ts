...
function checkSkipAsync(moduleName, options) {
    ...
    // исключение убрать в 23.3100 после исправления подзадачи в
    // https://online.sbis.ru/opendoc.html?guid=6fbcd3e1-f093-452e-b846-1d4671698444&client=3
    if (moduleName.indexOf('EOComplect') !== -1) {
        return false;
    }
    ...
    return new Promise<string | void>((resolve) => {
        Promise.resolve(
            ...
            .then((controlsHTML) => {
                ...
                if (checkSkipAsync(moduleName, options)) {
                ...
                getStateReceiver()
                    ...
                    .then(() => {
                        logger.info(
                            `waiting beforeMounts is over in ${
                                Date.now() - startSerialization
                            } ms`
                        );
                        resolve(controlsHTML);
                    });
...
export function mainRender(
    ...
    return new Promise((pageResolve) => {
        renderControls(moduleName, options).then(
            (controlsHTML: string = '') => {
                const fullData = aggregateFullData(
                    moduleName,
                    options,
                    controlsHTML
                );
