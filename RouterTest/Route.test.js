/* global assert, sinon */
define(['Router/Route', 'RouterTest/resources/controlManager', 'Router/Data'], function(Route, CM, RouterData) {
   var defaultOptions = {
      mask: 'name/:value'
   };
   ...
   describe('Router/Route', function() {
      ...
      it('fires on:enter if it is created when url matches the mask', function(done) {
         RouterData.setRelativeUrl('/unique/555-abc-def');
      ...
      it('does not fire events when switching between two matched states', function(done) {
         ...
         RouterData.setRelativeUrl(url1.state);
      ...
      it('correctly resolves the url parameters', function(done) {
         ...
         waitForLifecycle()
            .then(function() {
               RouterData.setRelativeUrl('/my/url?myparam=abc&test=true');
