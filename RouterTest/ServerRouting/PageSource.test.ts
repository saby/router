...
import { PageRenderer } from 'Router/_Builder/_Bootstrap/PageRenderer';
import {
...
describe('Router/_ServerRouting/PageSource', () => {
    beforeEach(() => {
        initResponseWrapper(createFakeResponse(), fakeRenderOptions);
    ...
    it('построение несуществующего модуля', () => {
        ...
        return new PageSource()
            .render(fakeRenderOptions, renderData, onSuccessHandler, onNotFoundHandler)
            .then((pageSource) => {
    ...
    it('успешное построение страницы', () => {
        const renderSpy = jest.spyOn(PageRenderer.prototype, 'render');
        ...
        return new PageSource()
            ...
            .then((pageSource) => {
                ...
                expect(renderSpy.mock.calls[0][1]).toHaveProperty('pageConfig', { data: 'data' });
            });
    });
