import { _prepareRoutes } from 'Router/_private/UrlRewriter';
...
describe('Router/UrlRewriterClass', () => {
    let urlRewriter: UrlRewriterClass;
    ...
    beforeEach(() => {
        urlRewriter = new UrlRewriterClass(_prepareRoutes(routerJson));
    });

    it('does not throw if router.json did not load', () => {
        // @ts-ignore
    ...
    it('правильно обрабатывается "constructor" в url', () => {
        // из объекта router.json необходимо искать части путей через hasOwnProperty
        // иначе будут получены методы и поля Object, например constructor
        expect(urlRewriter.get('/page/constructor/some')).toBe('/Module/page/constructor/some');
    });
    ...
    describe('forward rewrite', () => {
        ...
        it('returns urls as is if router.json did not load', () => {
            // @ts-ignore
            urlRewriter = new UrlRewriterClass(_prepareRoutes(null));
    ...
    describe('регулярные выражения в router.json', () => {
        ...
        it('/path/<regex> - в regex есть знак вопроса', () => {
            const url = '/path/foo-bar-baz';
            const result = '/Module/path/foo-bar-baz';
            expect(urlRewriter.get(url)).toBe(result);
        });
        it('/path/<regex>/<regex>', () => {
