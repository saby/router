...
function addTests(service_name: string, appAlias?: string) {
    ...
    describe('#calculateQueryState', () => {
        it('add param', () => {
            const newUrl = maskResolver.calculateQueryState({ param: 'value' });
            expect(newUrl).toEqual('/path?query=value&param=value');
        });
        it('replace params', () => {
            const newUrl = maskResolver.calculateQueryState({ query: 'newvalue' });
            expect(newUrl).toEqual('/path?query=newvalue');
        });
        it('replace params, url has trailing slash', () => {
            ...
            let newUrl = maskResolver.calculateQueryState(
                { param: 'newvalue' },
                '/site.ru/path/?param=value'
            );
            expect(newUrl).toEqual('/site.ru/path/?param=newvalue');

            newUrl = maskResolver.calculateQueryState(
                { param: 'newvalue' },
        ...
        it('clear params', () => {
            let newUrl = maskResolver.calculateQueryState({ replace: true });
        ...
        it('clear and add params', () => {
            const newUrl = maskResolver.calculateQueryState({ param: 'newvalue', replace: true });
            expect(newUrl).toEqual('/path?param=newvalue');
        });
        it('clear fragment', () => {
            const newUrl = maskResolver.calculateQueryState(
                { clearFragment: true },
        ...
        it('url with protocol', () => {
            const newUrl = maskResolver.calculateQueryState({ param: 'value' }, 'http://site');
