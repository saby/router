...
function addTests(service_name: string, appAlias?: string) {
    ...
    describe('#calculateState', () => {
        ...
        describe('в маске и значениях есть похожие слова', () => {
            it('test #1', () => {
                const newUrl = maskResolver.calculateState(
                    '/page/sg-site/:siteId/:pageUrl',
                    {
                        pageUrl: 'about',
                        replace: false,
                        siteId: 'menushop',
                    },
                    '/SabyGetOnline/page/sg-site/menushop'
                );
                expect(newUrl).toEqual('/SabyGetOnline/page/sg-site/menushop/about');
            });
            ...
            it('test #2', () => {
                ...
                expect(newUrl).toEqual('/menu/menushop/about');
            });
        });

        it('маска вида /:param/path и url вида /page/path', () => {
            // тут сложная ситуация, когда часть хвоста url совпадает с маской (/uc)
            // и при этом значение из url соответствующее параметру из маски начинается на этот же "хвост"
            // напр. url="/uchaly/uc" и mask="/:city/uc" - в url "uchaly" начинается на хвост в маске "uc"
            const newUrl = maskResolver.calculateState('/:city/uc', { city: 'moscow' }, '/uchaly/uc');
            expect(newUrl).toEqual('/moscow/uc');
        });

        it('маска вида /:param/path и url вида /some/page/path', () => {
            // тут ситуация, когда часть хвоста url совпадает с маской (/uc)
            // и при этом значение из url соответствующее параметру из маски начинается на этот же "хвост"
            // напр. url="/city/uchaly/uc" и mask="/:city/uc" - в url "uchaly" начинается на хвост в маске "uc"
            // тогда по ошибке получается результат - /moscow/uchaly/uc - а должен /city/moscow/uc
            const newUrl = maskResolver.calculateState('/:city/uc', { city: 'moscow' }, '/city/uchaly/uc');
            expect(newUrl).toEqual('/city/moscow/uc');
        });
    });
}
