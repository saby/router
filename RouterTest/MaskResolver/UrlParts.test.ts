...
describe('Router/_private/MaskResolver/UrlParts', () => {
    ...
    describe('UrlParts.extQuery', () => {
        it('extQuery only', () => {
            const urlParts = new UrlParts('/path/to/page/');
            const url = urlParts.join({ query: '?extSid=sid' });
            expect(url).toEqual('/path/to/page/?extSid=sid');
        });
        it('extQuery with other query', () => {
            const urlParts = new UrlParts('/path/to/page');
            const url = urlParts.join({ query: '?query=value&extSid=sid' });
            expect(url).toEqual('/path/to/page?query=value&extSid=sid');
        });
        it('extQuery multi sidQuery', () => {
            const urlParts = new UrlParts('/path/to/page/');
            const url = urlParts.join({ query: '?extSid=sid&extSid=sid' });
            expect(url).toEqual('/path/to/page/?extSid=sid&extSid=sid');
        });
        it('extQuery multi sidQuery with other query', () => {
            const urlParts = new UrlParts('/path/to/page');
            const url = urlParts.join({ query: '?query=value&extSid=sid&extValue=value' });
            expect(url).toEqual('/path/to/page?query=value&extSid=sid&extValue=value');
        });
    });
});
