...
function getFakeHistoryState(id: number, state: string, href: string): IHistoryState {
    return {
        id,
        state,
        href,
...
function addTests(service_name: string, appAlias?: string) {
    ...
    let historyInst: HistoryTest;
    // текущее состояние истории SPA переходов
    const initialHistorySPA = [
        urlPrefix + '/',
        urlPrefix + '/login',
        urlPrefix + '/login?oauth=saby',
        urlPrefix + '/login',
    ];
    ...
    beforeEach(() => {
        ...
        routerUrl = new RouterUrl(
            new WindowLocation(urlPrefix + '/signup'),
            UrlRewriter.getInstance()
        );
        historyInst = new HistoryTest(
            UrlRewriter.getInstance(),
            routerUrl,
            window.history,
        ...
        historyInst.setHistory([
            getFakeHistoryState(0, '/', urlPrefix + '/'),
            getFakeHistoryState(1, '/login', urlPrefix + '/login'),
            getFakeHistoryState(2, '/login?oauth=saby', urlPrefix + '/login?oauth=saby'),
        ]);
    ...
    it('returns the current history state', () => {
        ...
        expect(hstate.href).toEqual(urlPrefix + '/login');
    });
    ...
    it('returns the previous history state', () => {
        ...
        expect(hstate.href).toEqual(urlPrefix + '/');
    });
    ...
    it('returns the next history state', () => {
        ...
        expect(hstate.href).toEqual(urlPrefix + '/login?oauth=saby');
    });
    ...
    describe('#back', () => {
        ...
        test('вернет на правильное состояние, когда в истории есть несколько похожих состояний', () => {
            const newState = getFakeHistoryState(3, '/settings');
            historyInst.setHistory([
                getFakeHistoryState(1, '/settings'),
                getFakeHistoryState(2, '/areas'),
                newState,
                getFakeHistoryState(4, '/card'),
                getFakeHistoryState(4, '/card/new'),
            ]);
            historyInst.setHistoryPosition(4);

            // при нажатии стрелки "назад" в браузере
            historyInst.back(newState);

            const hstate = historyInst.getCurrentState();

            expect(hstate.id).toEqual(3);
            expect(historyInst.getHistoryPosition()).toEqual(2);
        });

        test('возврат на несколько состояний назад, когда в истории есть несколько переходов на одну страницу', () => {
            const newState = getFakeHistoryState(1, '/settings');
            historyInst.setHistory([
                newState,
                getFakeHistoryState(2, '/areas'),
                getFakeHistoryState(3, '/settings'),
                getFakeHistoryState(4, '/card'),
                getFakeHistoryState(4, '/card/new'),
            ]);
            // находимся на страницу "/card/new"
            historyInst.setHistoryPosition(4);

            // удержать стрелку "назад" в браузере и выбрать самую первую запись со страницей "/settings"
            historyInst.back(newState);

            const hstate = historyInst.getCurrentState();

            expect(hstate.id).toEqual(1);
            expect(historyInst.getHistoryPosition()).toEqual(0);
        });
    });
    ...
    describe('#forward', () => {
        ...
        it('переход вперед на другое новое состояние', () => {
            ...
            historyInst.forward(newState);
            const hstate = historyInst.getCurrentState();
            expect(hstate.id).toEqual(newState.id);
    ...
    describe('#replaceState', () => {
         ...
         */
        function testReplaceState(newState: IHistoryState, testHref?: string): void {
            const href = newState.href || testHref;
        ...
        it('замена в истории текущего состяния без передачи href', () => {
            ...
            testReplaceState(newState, urlPrefix + '/upage?navigation=profile');
        });
    });
    ...
    describe('onChangeHistoryState', () => {
        test('#back', () => {
            historyInst.back();
            const addItemInHistory = urlPrefix + '/';
            const expectedSpaHistory = [...initialHistorySPA, addItemInHistory];
            ...
            expect(onChangeHistoryStateSpy.mock.calls[0][0].href).toEqual(addItemInHistory);
        });

        test('#forward', () => {
            historyInst.forward();
            const addItemInHistory = urlPrefix + '/login?oauth=saby';
            const expectedSpaHistory = [...initialHistorySPA, addItemInHistory];
