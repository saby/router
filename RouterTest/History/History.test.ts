...
describe('Router/History', () => {
    const routerUrl = new RouterUrl(
        new WindowLocation('/signup'),
        UrlRewriter.getInstance()
    );
    const setSpaHistorySpy = jest.fn();
    ...
    describe('#back', () => {
        ...
        it('creates a new starting state if called in first position', () => {
            ...
            const expectedState =
                (typeof window !== 'undefined' && window.history.state.state) ||
                expectedHref;
    ...
    describe('#push', () => {
        ...
        it('adds a new state if current position is the last one', () => {
            ...
            expect(routerUrl.getStateUrl()).toEqual(
                '/upage?navigation=profile'
            );
    ...
    describe('SPA history', () => {
        ...
        test('максимальное количество SPA переходов ограничено', () => {
            ...
            expect(historyInst.getSpaHistory().length).toBe(
                SPA_HISTORY_MAX_LENGTH
            );
        });
    });
});
